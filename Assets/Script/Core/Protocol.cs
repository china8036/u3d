...
namespace Core
{
    public class Protocol
    {
        ...
        //处理完成的 msg记录
        public static Queue<QueueMsg> msgQueue = new Queue<QueueMsg>();
        ...
        {
            RecvMsg jsonMsg = JsonUtility.FromJson<RecvMsg>(msg);
            if (jsonMsg.code == 0)
            {//正常json code 不为0 除非没有code返回
                Debug.Log("无效的json数据:" + msg);
                return;
            }
            msgQueue.Enqueue(new QueueMsg(jsonMsg, msg));//此消息加入队列
